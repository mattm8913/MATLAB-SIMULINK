...
%   J. A. Piepmeier 
%   10 Nov 2010 
%   Added circle polygon example
%   Added image object
%   Added polygon/image intersection test
%   Modified code to look for length of handle variable h
%   Removed hard-coding that assumed only 3 objects
...
h(4) = fill(x4dat, y4dat, 'y');   % we use h(4) to represent object 4
%  Any additional objects are defined here
imicon=flipdim(imicon,1); %when images are shown on a regular axis, they appear upside-down.
h(5)=image(imresize(imicon,.3));
...
% 
xpos = width*rand(length(h),1) ;  % Random starting x for each of the 3 
ypos = ones(length(h),1)*height;       % Start at top of window, y = 10 

for i = 1:length(h)
    if (get(h(i),'Type')=='patch') % shapes are patch objects
    set(h(i), 'Vertices', [get(h(i), 'XData') + xpos(i), get(h(i), 'YData') + ypos(i)]); 
    else % images are not patch objects
        set(h(i), 'Xdata', get(h(i), 'XData') + xpos(i), 'YData', get(h(i), 'YData') + ypos(i)); %images don't have vertices
    end
end
...
%  speed 
speed = 50*rand(length(h),1) + 170;   % values between 170 and 220 units per second 
...
while (done == 0)  % keep going until the game is over 
   ...
   for i = 1:length(h)             % Three objects 
   ...
   for i = 1:length(h)
       if (get(h(i),'Type')=='patch') % shapes are patch objects
           ...
           xi=get(h(i), 'XData');
           yi=get(h(i), 'YData');
           xi_v=[xi(1) xi(1) xi(2) xi(2) xi(1)]; %X Vertices of Image
           yi_v=[yi(1) yi(2) yi(2) yi(1) yi(1)]; %Y Vertices of Image
           if (~isempty(polyxpoly(xc, yc,xi_v,yi_v)))
               disp('Hit!');
               text(0, 100, ['Hit by object ', num2str(i), '!!']);
               done = 1;  % game over
               break;
